        ...
        public string filterLatexSourceCodeUsingOpenDetex()
        {
            try
            {
                ...
                if (!Program.isPDfAccMode) //if not PDF Accessiblity Mode then Narrate Document in Math Mode
                {
                    ...
                    pdfAccessibleText = pdfAccessibleText.Replace(@"C Colon  Backslash opendetex is greater than", string.Empty);
                    pdfAccessibleText = pdfAccessibleText.Replace(@"Open Bracket tagged Close Bracket accessibility", string.Empty);
