...
namespace ExigoService
{
    public static partial class Exigo
    {
        public static IEnumerable<IAddress> GetCustomerAddresses(int customerID)
        {
            ...
            //                    c.OtherCountry
            //                }).FirstOrDefault();
            Customer Addresses = new Customer();
            using (var Context = Sql())
            {
                Context.Open();
                //Getting addresses for customer
                string sqlProcedure = string.Format("GetCustomerAddresses {0}", customerID);
                Addresses = Context.Query<Customer>(sqlProcedure).FirstOrDefault();
                Context.Close();
            }
        ...
        public static VerifyAddressResponse VerifyAddress(Address address)
        {
            ...
            try
            {
                if (address.Country.ToUpper() == "US" && address.IsComplete)
                {
                    ...
                    {
                        Address1 = address.AddressDisplay,
