...
namespace Common.Api.ExigoWebService
{
    public partial class ItemResponse
    {
        public static explicit operator ExigoService.Item(ItemResponse item)
        {
            ...
            model.Quantity = item.MaxAllowedOnOrder;
            model.MaxKitQuantity = item.MaxAllowedOnOrder;
            model.Price = item.Price;
            model.CurrencyCode = string.Empty;
            model.BV = item.BusinessVolume;
            model.CV = item.CommissionableVolume;
            model.InventoryStatus = item.InventoryStatus.ToString();
            ...
            model.TinyImageUrl = GlobalUtilities.GetProductImagePath(item.TinyPicture, item.ItemCode);
            model.SmallImageUrl = GlobalUtilities.GetProductImagePath(item.SmallPicture, item.ItemCode);
            model.LargeImageUrl = GlobalUtilities.GetProductImagePath(item.LargePicture, item.ItemCode);
