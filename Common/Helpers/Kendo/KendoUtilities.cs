    ...
    {        
        ...
        public static string GetSqlWhereClause(IEnumerable<FilterObject> filters)
        {
            ...
            foreach (var filter in filters)
            {
                ...
                //if (filter.Field1.Contains("Date"))
                //{
                //    filter.Value1 = string.Format("Format('{0}','M/d/yyyy') + ' 00:00:00.000'", filter.Value1);
                //}
        ...
        public static string GetSqlWhereClauseExpression(string field, string op, string param)
        {
            ...
            {
                case "eq":
                    return decimal.TryParse(param, out number) ? string.Format(" {0} = {1} ", field, param) : string.Format(" {0} = '{1}' ", field, param);
                ...
                case "doesnotcontain":
                    return string.Format(" {0} NOT LIKE '%{1}%' ", field, param);
