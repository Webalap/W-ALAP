...
{
    public class EnrolleeReward : IIndividualReward
    {
        #region Dependencies
        ...
        protected int? RewardPointsAccountId { get {return PointAccounts.EnrolleeRewards; } }
        ...
        {
            return products.Where(p => p.Discounts.Any(d => d.DiscountType == DiscountType.EnrolleeReward)).ToList();
        }
        ...
        protected IList<string> RedeemedandExpiredItemCodes()
        {
            ...
            //var rewards = RewardService.GetCustomerEnrolleeRewardDiscounts(CustomerId);
        ...
        public bool IsEligible(Customer customer, string siteType)
        {
            ...
            if (customer.CustomerID == 2) return false; 
            ...
            HttpContext.Current.Cache["EnrolleeRewardEligibility"] = isEligible;
        ...
        public void PopulateEligibleDiscounts(List<Product> products)
        {
            ...
            {
                if (!product.EligibleDiscounts.Any()) continue;
                var productDiscount = product.EligibleDiscounts.First();
        ...
        public IList<Product> GetActiveRewardProducts(IList<Product> productsInShoppingCart)
        {
            ...
            IList<string> enrolleeRewardProductsInCart = productsInShoppingCart.Where(p => p.Discounts.Any(d => d.DiscountType == DiscountType.EnrolleeReward)).Select(p => p.ItemCode).ToList();
            ...
            //    if (enrolleeRewardProductsInCart.Contains(product.ItemCode)) continue;
            //    var discount = new EnrolleeRewardDiscount()
            //    {
            //        CustomerExtendedDetailId = reward.CustomerExtendedDetailId,
            //        ItemCode = reward.ItemCode,
            //        DiscountAmount = reward.DiscountAmount,
            //        DiscountType = DiscountType.EnrolleeReward,
        ...
        {
            return discount.DiscountType == DiscountType.EnrolleeReward;
        }
        ...
        public RewardsAccount CalculatePointsAccount(IList<Product> products)
        {
            ...
            if (IsRewardPointsAccount)
            {
                ...
                foreach (var product in productWithDiscount)
                {
                    ...
                    appliedAmount += product.Discounts.Where(discount => discount.DiscountType == DiscountType.EnrolleeReward).Sum(discount => discount.AppliedAmount);
                }
        ...
        public void VerifyOrderIsValid(IOrder order)
        {
            ...
            IEnumerable<Product> productsWithMultipleRewards = rewardProducts.Where(p => p.Discounts.Count(d => d.DiscountType == DiscountType.EnrolleeReward) > 1);

            if (productsWithMultipleRewards.Count() > 1)
            {
                throw new ApplicationException("Enrollee Reward cannot be applied multiple times to the same product.");
            }

            //Check for any rewards that have been redeemed or expired
            foreach (var rp in rewardProducts)
            {
                var discount = (EnrolleeRewardDiscount)rp.Discounts.First();
                if (discount.HasBeenRedeemed)
                {
                    throw new ApplicationException(string.Format("Item {0} is not not eligible for Enrollee Reward", rp.ItemCode));
                }
            }
        }
        ...
        protected void AddProductsToPurchasedExtendedGroup(IEnumerable<Product> rewardProducts)
        {
            ...
            foreach (var product in rewardProducts)
            {
                ...
                var customerExtendedRequest = new CreateCustomerExtendedRequest { CustomerID = CustomerId, ExtendedGroupID = (int) CustomerExtendedGroup.EnrolleeRewards, Field1 = product.ItemCode, Field2 = DateTime.Now.ToString(CultureInfo.InvariantCulture), Field3 = 1.ToString(CultureInfo.InvariantCulture), Field4 = product.Discounts[0].AppliedAmount.ToString("c") };
        ...
        public void OrderPostProcessing(IOrder order)
        {
            ...
            foreach (var rp in rewardProducts)
            {
                var discount = (EnrolleeRewardDiscount)rp.Discounts[0];
