...
using LatexDocument.LatexModels;
...
namespace LatexDocument
{
    public class Document
    {
        ...
        /// </summary>
        /// <param name="FileName">Output file name without extension</param>
        public bool CreatePdf(string FileName, bool OpenAfterCompile = false)
        {
            LatexResponse rp = new LatexResponse();
            string FilePath = Path.Combine(FILE_FOLDER, string.Format("{0}.tex", FileName));
            Process p = Process.Start(LATEX_EXECUTABLE, string.Format("-aux-directory={0} -output-directory={1} {2} --halt-on-error", FILE_FOLDER, FILE_FOLDER, FilePath));
